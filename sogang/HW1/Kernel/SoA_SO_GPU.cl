...
__kernel void SoA_SO_GPU_grad(const __global uchar *input,
    ...
    grad[idx] = sum_x * sum_x + sum_y * sum_y;
}

__kernel void SoA_SO_GPU_norm(__global int *grad, __global uchar *output, const float min_grad, const float max_grad) {
    int y = get_global_id(1);
    int x = get_global_id(0);
    int H = get_global_size(1);
    int W = get_global_size(0);
    ...
    *(output + idx + gws) = intensity;
    *(output + idx + 2*gws) = intensity; 
}
/*
